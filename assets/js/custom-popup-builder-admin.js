( function( $ ) {
	...
	var CustomPopupBuilderBuilderAdmin = {

		init: function() {

			CustomPopupBuilderAdmin.importInit();

			if ( $( '#custom-popup-builder-library-page')[0] ) {
				CustomPopupBuilderAdmin.presetLibraryInit();
			}

			if ( $( '#custom-popup-builder-settings-page')[0] ) {
				CustomPopupBuilderAdmin.settingPageInit();
			}

		},

		importInit: function() {
			var $importButton = $( '#custom-popup-builder-import-trigger' ),
				$createButton = $( '#custom-popup-builder-create-trigger' );
			...
			var importTippy = tippy( [ $importButton[0] ], {
					html: document.querySelector( '#custom-popup-builder-import-form' ),
					...
					interactive: true,
					hideOnClick: true,
					theme: 'custom-popup-builder-light'
				}
			);

			var createTippy = tippy( [ $createButton[0] ], {
				html: document.querySelector( '#custom-popup-builder-create-form' ),
				//appendTo: $importButton[0],
				arrow: true,
				placement: 'bottom',
				flipBehavior: 'clockwise',
		...
		presetLibraryInit: function() {
			...
			Vue.component( 'preset-item', {
				...
				data: function() {
					return {
						modalShow: false,
						requiredPluginData: window.customPopupData.requiredPluginData
					}
				},
			...
			Vue.component( 'presetlibrary', {
				...
				created: function() {
					var libraryPresetsUrl         = window.customPopupData.libraryPresetsUrl,
						libraryPresetsCategoryUrl = window.customPopupData.libraryPresetsCategoryUrl,
						categories                = [],
						presets                   = [],
						vueInstance               = this;
					...
					eventBus.$on( 'createPopup', function ( presetId ) {
						...
						setTimeout( function(){
							vueInstance.$refs.customPopupLibraryForm.submit();
						}, 100 );
			...
			var libraryPage = new Vue( {
				el: '#custom-popup-builder-library-page',
			} );
		},
		...
		settingPageInit: function() {
			...
			Vue.component( 'settingsform', {
				...
				created: function() {
					var vueInstance = this,
						settings    = window.customPopupAdminData.settings,
						mailchimpApiData = window.customPopupAdminData.mailchimpApiData;
			...
			var settingsPage = new Vue( {
				el: '#custom-popup-builder-settings-page',
			} );
		}

	};

	CustomPopupBuilderAdmin.init();
